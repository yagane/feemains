...
async function displayTimeSlots() {
    ...
    const response = await fetch(`https://api.ton-domaine.com/check_appointment.php?date=${selectedDate.toISOString().split('T')[0]}`);
...
async function loadPrestations() {
    ...
    try {
        const response = await fetch('.https://api.ton-domaine.com/prestation.php');
...
document.getElementById('submit-reservation').addEventListener('click', async () => {
    ...
    try {
        ...
        const response = await fetch('https://api.ton-domaine.com/reservation.php', {
            method: 'POST',
            headers: {
